...
module.exports = function(options) {
    return through2.obj(function(file, enc, cb) {
		...
		var exp = /(\/\/\s?)?__inline\(['"](.*?)['"]\)/g;

        var str = file.contents.toString().replace(exp, (match, comment, filepath) => {
            return comment ? '' : loadFile(file.base, filepath, file.history[0], new FileLink(null, file.history[0]));
        });
...
function loadFile(base, p, from, fileLink = new FileLink) {
    ...
    if (fs.existsSync(p)) {
        ...
        str = str.replace(/(\/\/\s)?__inline\(['"](.*?)['"]\)/g, (match, comment, url) => {
            return comment ? '' : loadFile(path.dirname(p), url, p, childFileLink);
        });
